...
export class ProjectsController {
  ...
  @Req() req: Request,
  @Res() res: Response,) {
    const { logger, originalId: id } = res.locals;
    try {
    const data= await this.projectsService.create(id, createProjectDto);
    logger.info({
  ...
  async list(
    ...
    const { logger, originalId } = res.locals;
    try {
      const data = await this.projectsService.list(originalId, page, limit);
